...
# ADK API server.
#   --no_use_local_storage      -> in-memory session/artifact (Phase 1 stateless)
#   --trigger_sources=pubsub   -> registers POST /trigger/pubsub (Phase 5 wires
#                                 the Cloud Scheduler -> Pub/Sub push subscription
#                                 that delivers run triggers here)
#   --trace_to_cloud / --otel_to_cloud are added in Phase 7 once the
#     `opentelemetry-exporter-otlp` package is installed; enabling them here
#     without that package crashes the container on boot.
CMD ["adk", "api_server", \

...
_VERIFICATION_INSTRUCTION = specialist_instruction(
    ...
    output_description=(
        ...
        "      - Bank matching uses ONLY the invoice STATED total field, within\n"
        "        ±$0.02. NEVER match a bank row against subtotal+tax recomputed\n"
        "        from line items.\n"
        "      - matched_amount is the POSITIVE magnitude (|bank amount|) in\n"
        "        invoice currency — never a signed bank debit.\n"
        "      - If the stated total differs from subtotal+tax by more than\n"
        "        $0.02, that is ITSELF an amount_mismatch discrepancy (flag it;\n"
        "        invoice_value=stated total, bank_value=matching bank row amount\n"
        "        if vendor/number/date align, else null). NEVER treat the\n"
        "        recomputed subtotal+tax as the match basis.\n"
